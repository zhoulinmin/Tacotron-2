...
class Tacotron():
	...
	def add_optimizer(self, global_step):
		...
		for i in range(hp.tacotron_num_gpus):
			...
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('optimizer') as scope:
					update_vars = [v for v in self.all_vars if not ('inputs_embedding' in v.name  or 'encoder_' in v.name or 'decoder_' in v.name)] if hp.tacotron_fine_tuning else None
